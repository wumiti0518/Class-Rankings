...
import { Trophy, Download, RotateCcw, GripVertical, MoreVertical, UserCog, Send, FileSpreadsheet, History, Trash2, Clock, Save, Edit2, ChevronDown, ChevronUp, Plus, Minus } from 'lucide-react';
...
export default function App() {
  const [currentGroups, setCurrentGroups] = useState<Group[]>(() => {
    const saved = localStorage.getItem('class_61_data');
  ...
  const [logs, setLogs] = useState<LogEntry[]>(() => {
    const saved = localStorage.getItem('class_61_logs');
  ...
  useEffect(() => {
    ...
    // One-time cleanup: remove "吾米提" if he exists in currentGroups
    const hasWumiti = currentGroups.some(g => g.members.some(m => m.name === '吾米提'));
    if (hasWumiti) {
      const cleanedGroups = currentGroups.map(g => ({
        ...g,
        members: g.members.filter(m => m.name !== '吾米提')
      }));
      saveData(cleanedGroups);
      addLog('move', '系统自动清理了测试成员：吾米提');
    }
  ...
  const saveData = (groups: Group[]) => {
    ...
    localStorage.setItem('class_61_data', JSON.stringify(groups));
  };
  ...
  const addLog = (type: LogEntry['type'], message: string, data?: Group[]) => {
    ...
    setLogs(prevLogs => {
      ...
      localStorage.setItem('class_61_logs', JSON.stringify(updatedLogs));
      return updatedLogs;
    });
  };
  ...
  const clearLogs = (e: React.MouseEvent) => {
    ...
    if (window.confirm("确定要清空所有活动日志吗？")) {
      setLogs([]);
      localStorage.setItem('class_61_logs', JSON.stringify([]));
  ...
  const renameLog = (e: React.MouseEvent, logId: string, currentMessage: string) => {
    e.stopPropagation();
    const newName = window.prompt("重命名日志：", currentMessage);
    if (newName !== null && newName.trim() !== "") {
      setLogs(prevLogs => {
        const updatedLogs = prevLogs.map(log => 
          log.id === logId ? { ...log, message: newName } : log
        );
        localStorage.setItem('class_61_logs', JSON.stringify(updatedLogs));
        return updatedLogs;
      });
